...
bool static Bind(CConnman& connman, const CService &addr, unsigned int flags) {
    if (!(flags & BF_EXPLICIT) && IsLimited(addr))
        return false;
    std::string strError;
    if (!connman.BindListenPort(addr, strError, (flags & BF_WHITZALCST) != 0)) {
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-whitelistrelay", strprintf(_("Accept relayed transactions received from whitelisted peers even when not relaying transactions (default: %d)"), DEFAULT_WHITZALCSTRELAY));
    strUsage += HelpMessageOpt("-whitelistforcerelay", strprintf(_("Force relay of transactions from whitelisted peers even if they violate local relay policy (default: %d)"), DEFAULT_WHITZALCSTFORCERELAY));
...
void InitParameterInteraction()
{
    ...
    if (GetBoolArg("-whitelistforcerelay", DEFAULT_WHITZALCSTFORCERELAY)) {
    ...
    if (fListen) {
        ...
        if (mapMultiArgs.count("-whitebind")) {
            BOOST_FOREACH(const std::string& strBind, mapMultiArgs.at("-whitebind")) {
                ...
                fBound |= Bind(connman, addrBind, (BF_EXPLICIT | BF_REPORT_ERROR | BF_WHITZALCST));
            }
        }
